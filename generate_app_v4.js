...
function App() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'kanban' | 'projects' | 'users' | 'gestores'>('dashboard');
  ...
  const [newProject, setNewProject] = useState({ title: '', description: '', gestorId: '' });
  ...
  const handleCreateProject = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewProject({ title: '', description: '', gestorId: '' });
      fetchProjects();
      fetchDashboard();
      ...
      <div className="main-content">
        ...
        {activeTab === 'kanban' && (
          <div>
            ...
            {/* FORMULARIO DE TAREFA LIBERADO PARA TODOS */}
            <div className="card" style={{ marginBottom: '32px' }}>
            ...
            {(currentUser?.role === 'SUPER_ADMIN' || currentUser?.role === 'MANAGER') && (
              <div className="card" style={{ marginBottom: '32px' }}>
                ...
                <form onSubmit={handleCreateProject}>
                  ...
                  <div className="form-group">
                    <label className="form-label" htmlFor="projectGestor">Gestor Responsável</label>
                    <select
                      id="projectGestor"
                      name="projectGestor"
                      className="form-input"
                      value={newProject.gestorId}
                      onChange={e => setNewProject({ ...newProject, gestorId: e.target.value })}
                      required
                    >
                    ...
                    <div key={p.id} className="document-item animate-hover">
                      <div>
                        ...
                        <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                          ...
                          {p.gestor && \` • Gestor Responsável: \${p.gestor.name}\`}
                        </div>
                      </div>
        ...
        {activeTab === 'gestores' && (currentUser?.role === 'SUPER_ADMIN' || currentUser?.role === 'MANAGER') && (
          <div>
            <div style={{ marginBottom: '32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <h1 style={{ fontSize: '2rem', fontWeight: 700 }}>Gestão de Gestores</h1>
                <p style={{ color: 'var(--text-muted)', marginTop: '4px' }}>Cadastre e gerencie a hierarquia administrativa do portal.</p>
              </div>
            ...
            <div className="card">
              <h3 style={{ marginBottom: '20px', fontWeight: 600 }}>Gestores Cadastrados ({gestores.length})</h3>
              <div className="document-list">
                {gestores.length > 0 ? (
                  gestores.map(g => (
                    <div key={g.id} className="document-item animate-hover" style={{ opacity: g.isActive === false ? 0.5 : 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                        <div className="avatar" style={{ width: '40px', height: '40px', fontSize: '1rem', background: g.isActive === false ? '#4b5563' : 'var(--primary-dark)' }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '2px' }}>
                            {g.email} {g.cargo && \` • \${g.cargo}\`} • Vinculado a: {g._count?.users || 0} usuários, {g._count?.projects || 0} projetos
                          </div>
                        </div>
                      </div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <button 
                          ...
                          onClick={() => handleDeleteGestor(g.id, g.name)}
                          style={{ background: 'rgba(239,68,68,0.05)', border: 'none', borderRadius: '4px', padding: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                        >
                          <Trash2 size={16} style={{ color: 'var(--danger)' }} />
                        </button>
                  ...
                  <p style={{ color: 'var(--text-muted)' }}>Nenhum gestor cadastrado.</p>
                )}
              </div>
            </div>
          </div>
        ...
        <div style={{
          ...
          <div className="card" style={{
            ...
            <form onSubmit={handleSaveUser}>
              ...
              <div className="form-group">
                <label className="form-label" htmlFor="modalGestor">Gestor Responsável</label>
                <select 
                  id="modalGestor"
                  name="modalGestor"
                  className="form-input"
                  ...
                  required
                >
                  <option value="">Selecione o gestor deste colaborador...</option>
                  {gestores.filter(g => g.isActive).map(g => (
                    <option key={g.id} value={g.id}>{g.name}</option>
                  ))}
                </select>
...
console.log('App.tsx generated successfully with Gestor Cargo support!');
