...
const LOG_FILE_PATH = path_1.default.join(process.cwd(), 'logs', 'email_notifications.log');
...
`;
    try {
        fs_1.default.appendFileSync(LOG_FILE_PATH, logMessage, 'utf8');
...
class EmailService {
     ...
     */
    static async sendTaskAssignmentEmail(userIds, taskId) {
     ...
     */
    static async sendTaskCompletionEmail(taskId, completedByUserId) {
        try {
            ...
            const completedByUser = await prisma_1.default.user.findUnique({
            ...
            const addRecipient = (name, role, email) => {
     ...
     */
    static async sendUserWelcomeEmail(userId, plainTextPassword) {
        try {
